...
- 2026-09-21 后续反馈：最小化重建 FramePool 约需 4 秒，已按用户选择改为暂停采集但保留有效实例，
  协议升至 3。真实本桌面测试窗口恢复显示 190 ms，暂停停采及释放边界回归通过，已同步测试目录；
  用户随后反馈实测“没有太大的问题”并要求提交；该反馈不扩展为本轮长期资源观察验收。
...
- [V2 设计讨论](issues/maanop-updater-v2.md) 已逐轮明确文件所有权、失败语义、GUI → Engine handoff seam、
