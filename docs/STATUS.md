...
2026-09-24：任务搜索改为标题旁放大镜按需展开并聚焦，默认不占行；入口切换为关闭按钮，
关闭/Esc 清空查询、恢复分类状态并收起，整体折叠任务区也清空搜索，点击任务不会自动收起搜索。
Release GUI 自包含发布与 project-only 自检通过，补验默认隐藏、入口打开、Esc、关闭按钮与折叠行为，
窄窗口离屏截图检查通过。已备份并同步目标 `D:\MaaNOP-win-x86_64-v2.4.0` 的 GUI DLL；
6 个 GUI 文件哈希一致，分组 interface、旁边备份及 config 哈希未变。
旧 DLL 保存在 `artifacts/task-search-sync-backup-20260924-163340`，真实交互效果待用户查看。

2026-09-24：`task` 分支 GUI 自包含发布成功，备份后同步至 `D:\MaaNOP-win-x86_64-v2.4.0`。
6 个变化的 GUI 文件 SHA256 校验一致，interface.json 与已有 config 文件哈希未变。
备份为 `artifacts/task-ui-sync-backup-20260924-162645`；目标 PI 尚无 group，默认仍平铺显示。
本轮未启动目标程序，等待用户查看实际效果；构建和自检证据见下一条。

2026-09-24：简化未提交的任务分类改动：展开/收起共用非搜索状态判断，合并自检中的筛选与存在性判断，
整理新任务工作区的 XAML 层级缩进；分类、搜索、配置及执行行为保持不变。
Release GUI 构建 0 警告/错误，完整 GUI 自检、改动行 120 列及 diff 空白检查通过。
本轮未执行真实游戏/分身交互，未同步安装目录或提交。

2026-09-24：`task` 分支接入可用任务分类 UI：按 PI group 顺序提供轻量折叠标题、组内自动换行，
未匹配分类的任务置于“未分组”，多组任务共享已添加状态。固定搜索区按任务名称/标签筛选，
搜索时展开匹配组，清空后恢复原折叠状态；空结果有提示。可用任务内容最高为工作区高度三分之一、
不超过 280 DIP，小目录自然收缩；执行计划独立滚动，整体可用任务折叠入口保留。
新手指引适配执行计划的新滚动容器；group 解析仍在既有 ProjectInterfaceLoader，相关测试合入既有自检文件。
Release GUI 构建 0 警告/错误，完整 GUI 自检通过；覆盖 12 分类/38 任务、搜索恢复、多组状态、独立滚动，
检查了 920×640 和 1440×900 离屏截图及长任务名换行。未执行真实游戏/分身交互或同步测试安装目录。

2026-09-24：ProjectModel 支持 PI 顶层 `group` 与 `task.group` 解析，暴露分类名称、显示文案、图标路径、
默认展开状态及有序多组归属；缺省为空，默认展开 true，拒绝重复分类名及错误字段类型。
未知组引用保留供后续 UI 归类；本轮不实现分类 UI、import 或国际化解析，不改变配置格式与执行顺序。
Release GUI 构建 0 警告/错误，`--self-test --project-only` 通过，覆盖分组默认值、空数组、多组、非法输入及
配置/执行内容隔离；源 PI 摘要仍随文件内容变化。未运行 MXU 或新增 GUI 分类交互验收。
...
通过本地 `artifacts/preview-simplify-check` 对本轮 Worker 构建补跑除双进程外的 12 项自检，
包含停采/实例复用、窗口失效、过期释放及协议/Stop 隔离。本轮未重跑已知超时的双进程测试或实机交互。
改动行 120 列与 diff 空白检查通过；保留测试文件原有一处超长行，未扩展无关格式化。
...
内存、handle/thread、截图 Controller 未观察到明显持续增长或积累，任务未因 Preview 明显退化；
本次没有采样明细，不推导数值指标。完整勾选清单、来源与剩余专项见
[2026-09-21 验收记录](ACCEPTANCE-2026-09-21.md)。本次仅同步文档，未重跑构建或交互测试。

## 历史实施与验证记录

下列记录保留各轮当时的证据和限制；已补验范围以当前阶段及对应最新验收记录为准。
