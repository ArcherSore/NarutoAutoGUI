# 持久化两阶段 Worker Admission，并只优雅关闭 Idle Worker
...
Admission Record 只在封闭条件下删除：启动明确失败且确认未产生 Worker；Pending grace period 到期且确认无匹配 Worker；已记录 PID 被确认不存在或身份明显不匹配；`worker.shutdown` 或强制终止后确认旧 PID 已退出；Child Session 注销成功；或人工恢复明确废弃旧 identity。IPC 暂时断开、Snapshot 超时或 Worker 尚未完成启动均不是删除理由。
...
首版 `worker.shutdown` 只服务于没有 activeRun 的 Worker 替换。接受后 Worker 进入 Stopping、停止接受新请求、flush 必要日志、关闭 IPC 并正常退出；activeRun 非空时返回 `operation_not_allowed`，必须先停止 Run。GUI 在固定 timeout 内等待并验证旧 PID 退出，必要时只针对已验证 PID 强制结束；确认退出后删除 Admission Record，生成新的 instance ID/token/context 并启动新 Worker。游戏和 Child Session 始终保持。

结束整个桌面分身不要求 `worker.shutdown`。activeRun 且 IPC 可用时先发 `run.stop` 并有界等待；停止超时或失败时，GUI提示“停止未确认，将通过注销 Session 强制结束执行环境”，然后仍继续 WTS Logoff，不能永久阻止用户结束。只有 WTS Logoff 失败才保留明确错误状态。MFAAvalonia 不进入任何自动启动、恢复或替换链路。
