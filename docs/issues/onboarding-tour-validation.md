...
用户确认新用户无配置启动、默认任务出现并展开、四步 Tour、Skip/Esc/replay 与任务配置不被改动均通过。
老用户直接加载旧配置且不自动弹 Tour，多配置保留以及切换、重命名、删除、参数保存均正常。
按用户实机反馈记录，详见 [统一验收清单](../ACCEPTANCE-2026-09-21.md)。本次未重新操作 GUI 或核对文件哈希。
下方部署与日志检查保留历史时间线；多 DPI、焦点遍历、标题栏/托盘等清单外专项仍未逐项确认。
