...
2026-09-21：用户确认完整运行闭环和至少 30 分钟 Preview 观察完成、无明显问题，记为实机通过。
覆盖准备后未启动任务即显示、运行中持续、Stop 后保持、再次执行、隐藏分身、游戏关闭清空/等待及重开恢复。
GUI/Worker 内存、handle/thread 和截图 Controller 未观察到明显持续增长或积累，任务未明显退化。
来源及逐项结果见 [统一验收记录](../ACCEPTANCE-2026-09-21.md)；本次未提供数值采样或构建身份。
固定后端开关对照、其他窗口/重连边界和本地双进程帧测试超时仍保留，工单不据此全部关闭。

## 历史实现与验证

以下为各轮当时的结论；已确认的运行闭环和 30 分钟观察不再待验收。
...
以下为历史计划；运行闭环和至少 30 分钟定性资源观察已于 2026-09-21 获用户确认。
未提供的量化记录及清单外专项见上方当前结论，不重复要求已完成项目。
